...
public class PageServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        sql.append("select * from artworks where ").append(method[0]).append(" LIKE '%").append(input).append("%'");
        for (int i =1; i<method.length;i++){
            sql.append("AND").append(method[i]).append(" LIKE '%").append(input).append("%'");
        }


        PageInfo<Painting> news = service.findAlls(Painting.class,"paintings",
