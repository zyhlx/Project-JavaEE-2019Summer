...
public class PageDAOImpl implements IPageDAO {
    ...
    public <T> List<T> findAlls(Class<T> clazz,String tableName, int pageNum, int pageSize) {
        String sql = "select ImageFileName imageFileName,Title title,Description description,PaintingID paintingID,MSRP msrp  from "+tableName+"  ORDER BY MSRP desc limit ?,?";
        // 创建一个集合来保存
        return DBUtil.get(clazz,sql,pageNum,pageSize);
    }
    @Override
    public int getTotalCount() {
        String sql = "select count(*) as count from paintings";
        int count = 0;
        try {
