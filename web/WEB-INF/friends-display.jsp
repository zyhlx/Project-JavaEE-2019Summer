...
<main>
<h1 class="">
    Friends <a href="./friend.search">  <button class="btn btn-search btn-primary" type="button"> 搜索</button> </a>
</h1>
...
<c:forEach items="${friends}" var="friendItem" >
    <div class="row border user-result">
        ...
        <div class="col-8 panel panel-default">
            <div class="panel-heading user-name"><a href="userDetail.display?userID=<c:out value="${friendItem.client.userID}"/>">${friendItem.client.username}</a></div>
            <table class="table">
                ...
                <tr><td class="info-title">最近收藏</td><td><c:forEach items="${friendItem.client.favours}" var="favourItem">
                    <a href="./paintingDetail.display?paintingID=${favourItem.painting.paintingID}"> ${favourItem.painting.title} </a>
                </c:forEach></td>
                </tr>
            </table>
        </div>
